...
def time_series_xform(df, subjects_col, time_series_ref_cols: list, time_series_data_cols):
    ...
    for subject in subjects_list:
        subject_idx = df_ts.index[df_ts[subjects_col] == subject]
        sorted_time_series_data_cols = df.loc[df[subjects_col] == subject].sort_values(by=time_series_ref_cols)
        for time_series_col in time_series_ref_cols + time_series_data_cols:
            df_ts.at[subject_idx, time_series_col] = list(sorted_time_series_data_cols[time_series_col])
